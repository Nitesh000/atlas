...
function UsagePage() {
  ...
  if (orgLoading || usageLoading) return <div className="p-8 text-muted-foreground animate-pulse">Loading usage...</div>;
  ...
  const percentage = Math.min(100, Math.max(0, (usage.apiCallCount / usage.limit) * 100));
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Current billing cycle: {usage.monthYear}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex justify-between items-end">
              <div>
                <span className="text-4xl font-extrabold">{usage.apiCallCount.toLocaleString()}</span>
                <span className="text-muted-foreground ml-2">/ {usage.limit.toLocaleString()}</span>
              </div>
            ...
            {percentage >= 100 && (
              <div className="p-3 bg-destructive/10 text-destructive rounded-lg text-sm flex items-center gap-2">
                <AlertCircle className="h-4 w-4" /> You have reached your monthly API limit. Requests are halted.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Upgrade to unlock higher capacity.</CardDescription>
          </CardHeader>
