...
_Copy and paste the following prompt into Stitch, v0, Claude Artifacts, or your UI generator of choice to build the complete design system and layouts for Atlas._
...
- **Color Palette:**
  - _Primary:_ Electric Indigo/Violet (e.g., `hsl(250, 89%, 65%)`). Should glow on hover.
  - _Muted:_ Zinc-900 for secondary backgrounds.
  - _Status:_ Neon green for `completed`, Amber for `crawling` (with a pulsing animation), Crimson for `failed`.
...
- **Panel 1: Data Ingestion (Websites)**
  ...
  - _Crucial:_ The "Status" column must feature animated badges (e.g., a pulsing yellow dot next to "Crawling", a solid green checkmark for "Completed").
- **Panel 2: API Keys**
...
- **Open State:**
  ...
  - _Animation:_ Framer motion layout transitions when the widget expands/collapses.
